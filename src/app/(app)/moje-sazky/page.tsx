...
export default async function MojeSazkyPage() {
  const session = await auth();
  if (!session?.user) redirect("/login");

  const t = await tournamentService.getActive();
  const data = await loadMyBetsData(session.user.id);
      ...
      {data.parlayCards.length === 0 && data.groupList.length === 0 ? (
        <Card>
          <CardContent className="pt-6 text-sm text-muted-foreground">
            Zatím jsi nevsadil. Otevři Sázení a začni.
          </CardContent>
        </Card>
      ) : (
        <div className="space-y-6">
          {data.parlayCards.length > 0 && (
            <section className="space-y-3">
              <h2 className="text-base font-semibold uppercase tracking-wider text-muted-foreground">
                Akumulátory
              </h2>
              <div className="space-y-3">
                {data.parlayCards.map((p) => (
                  <Card key={p.id}>
          ...
          {data.groupList.length > 0 && (
            <section className="space-y-3">
              ...
              <BetsByMatch groups={data.groupList} />
            </section>
          )}
        </div>
      )}
